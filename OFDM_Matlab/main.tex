...
% 基本的有 $...$ 或是 \[...\] (Tip：$$...$$ 為舊版，請使用\[...\])
%行內的文字和句中的 $a+b=c$ 數學式之間會加一個空格，比較好看。
%比較短的式子才加在句中，比較長或比較重要的式子，比如 $\sum_{k=1}^n k^2 = 1+4+\cdots+n^2 = \frac{n(n+1)(2n+1)}{6}$ 放在式子中會不好看，所以要用反斜中括號
%\[\sum_{k = 1}^n k^2 = 1 + 4 + \cdots + n^2 = \frac{n(n+1)(2n+1)}{6}\]
%放在中間。
%接下來我們來看各種數學式子的打法。

\section{數學式}
例子：Functional equation of zeta function.
\[\zeta (s)=2^{s}\pi ^{s-1}\sin \left({\frac {\pi s}{2}}\right)\Gamma (1-s)\zeta (1-s)\]
再來，讓我們一一解釋會用到的物件。

\subsection{基礎符號}
% 基礎運算：顯示的跟打的一樣 +-*/=()[]
% 數學環境內的空白會被忽略 (數學內要使用\, 或 \quad)
...
% 為了減少空格量的不穩定，不要無緣無故加上空格較佳

\subsection{上下標}
% 上下標與大括號：大括號內視為一組
% 上下標順序對TeX沒有差，但通常會視到底應該是 (a_1)的^2 還是 (a^2)的_1 決定哪個放前面。
\[(x_1+x_2)^2 = x_1^2 +2x_1x_2+x_2^2\]
\[\lambda_1,\lambda_2,\ldots,\lambda_{20}\]

\textbf{練習：}大括號的不同。試著打出
\[a_5\times 10 \neq a_{5\times 10}\]
%-----練習區-----%
\[
    % 在這裡試試
...
%-----練習區-----%
\subsection{希臘字母}
% 希臘字母：加上反斜線，大寫希臘字母為首字大寫，以及一些變體。
\[ax^2+bx+c = a(x-\alpha)(x-\beta)\]

\textbf{練習：}Uniform continuity. 試著打出
\[\forall \varepsilon>0,\, \exists \delta>0,\, [|y-x|<\varepsilon \implies |f(y)-f(x)|<\varepsilon]\]
%-----練習區-----%
\[
    % 在這裡試試
\]
...
\subsection{特殊符號}
% 特殊符號：許多不同意義的數學符號
\[X \subseteq Y\cup Z \iff \forall x \in X,\, x\in Y \lor x\in Z.\]

\[B\simeq A\oplus C \implies 0\to A\to B\to C\to 0\]

\textbf{練習：}Existence of empty set. 請打出
...
\[\sqrt{x^2+y^2+z^2}\leq \sqrt[4]{3(x^4+y^4+z^4)}.\]

\textbf{練習：}Schr\"odinger equation for free particle in one dimension. 打出
...
\subsection{數學字體}
% 數學字體 (\mathbb{},\mathcal{},\mathfrak{})
$\mathbb{F}_p:= \mathbb{Z}/p\mathbb{Z}$, where $p$ is a prime number.
\[\mathbb{Q}=\left\{\frac{m}{n}:m,n\in\mathbb{Z},n\neq 0\right\}\]

\textbf{練習：}Theorem of intersecting family. 打出以下文字：\\
If $\mathcal{F} \subseteq 2^X$ and for each $S,T\in \mathcal{F}, |S\cap T|$ is even, then $|\mathcal{F}|\leq 2^{\lfloor n/2 \rfloor}$.
%-----練習區-----%
\[
    % 在這裡試試
\]
...
\subsection{點點點}
% 點點點：\ldots 於列出每一項，中間用逗號隔開時使用；\cdots 則於其他大部分情況使用。

\[S^{n}:= \{ (x_0,x_1,\ldots,x_n)\in \mathbb{R}^{n+1} \,|\, x_0^2+x_1^2+\cdots+ x_n^2 = 1\}\]

\subsection{重音符號}
% 重音符號：符號上方的東西
\[\ddot{u} = \frac{d^2}{d^2x}u
,\quad
...
文字內 $\sum_{n=1}^\infty \frac{1}{n^2}=\frac{\pi^2}{6}$,
\[\prod_{n=1}^{\infty} \frac{1}{n^2}=0,\]
文字內 $\sum\limits_{n=1}^\infty \frac{1}{n^2}=\frac{\pi^2}{6}$. % 加了\limits

\[ \left(\frac{n}{k} \right)^n \leq \binom{n}{k} < \left(\frac{ne}{k}\right)^2 \approx \frac{n^n \cdot (2.718281828)^n}{k^n},\quad n>k\geq 0.\]

\[\int_0^1 x\, \mathrm{d}x =  \left. \frac{x^2}{2} \right|_0^1.\]

\textbf{練習：}Set theory. 打上
    \[\bigcap_{\alpha\in \Lambda} \left( \bigcup_{\beta\in \Lambda_\alpha} S_{\alpha,\beta} \right) 
    = \bigcup_{\{\beta_\alpha\}: \alpha\in \Lambda, \beta_\alpha\in \Lambda_\alpha} \bigcap_{\alpha \in \Lambda} S_{\alpha,\beta_\alpha} \]
%-----練習區-----%
\[
    % 在這裡試試
\]
...
\subsection{其他}
% 英文引號請不要直接用 "word"，請用 ``word''
% 橫線有三種
英文引號【A good ``quote,'' a bad "quote".】與橫線【There are some mind-blowing equations on pages 15--96. This is a sentence --- with comment. 還有中文破折號-----對～】

\subsection{文字運算子}
% $cos x$ 的意思是 c,o,s,x 相乘，遇到類似的請加上反斜線
\[\cos(\phi+\psi) = \cos\phi \cos\psi - \sin\phi \sin\psi\]
\[\lim_{n\to\infty} \log_n 2 = 1.\]

% 模運算 mod 的使用
\[a^p \equiv a \pmod p\]

% \operatorname{}: 使用自定義的文字運算子
\[\operatorname{coker} f := H / \operatorname{Im} G\]

\textbf{練習：}Rank-nullity theorem. 打出
...
\subsection{特殊字元}
% 特殊字元：依下列方式打出
The symbols  \&, \%, \$, \#, \_, \{, \}, \~{}, \^{}, and \textbackslash{} cannot be input directly. Here are the ways to type them.

\subsection{最終練習}
打出一開始出現的例子(Functional equation of zeta function.)
%-----練習區-----%
\[
    % 在這裡試試
\]
...
\section{更多網路上的資源}
首先，「\href{https://detexify.kirelabs.org/classify.html}{DeTeXify}」是一個在框框中畫上符號就會給出 LaTeX 指令的網站。注意，有指令會需要相對應的套件（package）！再來，「\href{https://ctan.org/}{The Comprehensive TeX Archive Network (CTAN)}」是最全面的 TeX 資料網，其中包含了符號大全「\href{https://tug.ctan.org/info/symbols/comprehensive/symbols-a4.pdf}{The Comprehensive LaTeX Symbol List}」，以及如何製作符號的教學與各種套件的使用說明書等等。還有，「\href{https://tex.stackexchange.com/}{LaTeX Stack Exchange}」是知名的討論版。最後，是真的不短的 LaTeX 手冊「\href{http://tug.ctan.org/info/lshort/english/lshort.pdf}{The Not so short Introduction to LaTeX2e}」。
